...
사전 선언: step 500 추론 렌더는 네 런에서 sha가 동일하고 warm-up 동안 `enc_p`/`dp`/`flow`/`dec`가
전부 동결이므로, **추론@500은 릴리스 사전이 한국어 음소를 렌더한 것이고 콤이 없어야 한다**
(격자 초과 < +1). 재구성@500이 울리는지는 미지수이며, 울리더라도 기록은 "warm-up 종료 시점에
이미 존재"까지다 — 양성 대조가 없어 원인 귀속은 할 수 없다. 어떤 지표 하나로도 원인을 배제하거나
처방을 확정하지 않는다. **새 장기 학습은 이 진단과 후속 청취 이후에 별도로 승인받는다.**
