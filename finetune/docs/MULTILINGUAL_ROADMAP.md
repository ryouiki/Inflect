...
**대조 실험**: 같은 커밋·같은 시드에서 `--mel-loss-legacy-floor` 하나만 다른 두 arm을 1,500 step.
판정은 step 500/1000/1500의 40행 대응 문장에서 `matched − legacy` 문장별 차이로 하고,
큰 개선 / 부분 개선 / 불확실 / 악화 4단으로 기록한다. **`<2 dB` 미달을 원인 기각으로 쓰지 않는다.**
