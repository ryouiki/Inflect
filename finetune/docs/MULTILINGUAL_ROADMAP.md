...
### 3.5 W1500 — posterior warm-up 스케줄 짧은 탐색 런 (2026-09-06) — **청취 대기**
...
진입 시점 상태가 **뒤바뀌었다**. 그러나 **잠김은 계기이지 말소리 판정이 아니므로** 여기서 결론을 내지 않는다.
`listening/warmup-20260906/`(3문장 × 5시스템 + catch, 공통 −27.03 dBFS) 판정 대기이며, 판정은 문장별로
ⓐ 진입 직전 재구성 · ⓑ W 추론 · ⓒ 비교군 추론의 **같은 문장 안 대비**로 한다(보존 관측 / 실패 / 미결).
한국어 발화·이해는 2차 결과로 따로 보고한다. 다음 런은 그 뒤 별도 계획.
