...
### 3.7 말소리 성립 시점과 전환에서 잃은 것 (2026-09-07) — **JSUT는 2k에 이미 말소리 · 적응이 있던 능력을 지웠다**
...
**페이지 A(JSUT 화자, catch 잡음 바닥 0단계)** — 문장별 자연성·결함:
...
**같은 예산(2,000 step)에서 KO·Arona 런은 말소리가 없었다** — "짧아서 아직 안 된 것"만으로는 설명되지 않는다. 다만 두 조건은
데이터량·목표 음역·시작점이 함께 다르므로 **어느 축의 효과인지 말하지 않는다.** 디코더 해제(3,000) **전에 이미 말소리**였고
해제 직후 자연성이 뛰는 관측이 있으나 **디코더 인과는 확정하지 않는다.** 원본@2000이 말소리로 확인됐으므로 **다음 라운드에서
replay@2000 비교가 처음으로 유효해진다.**

**페이지 B(JA Arona 화자, catch 잡음 바닥 1단계 — 1등급 차이는 읽지 않는다)**:
...
- **B-loss 2/3 → "ADAPT 직전에는 말소리였고 같은 문장에서 적응 후 사라졌다."** 동결 사전은 Arona 텍스트를 자연성 3~4·결함 1의
  ...
  **적응이 있던 능력을 지웠다** — 같은 문장·같은 라운드에서 확인된 첫 사례.
- **B-step0 2/3이 `X→X→X` → "계승 posterior는 전환 시점부터 Arona 음성을 말소리로 복원하지 못했다."** warm-up 탓이 아니다.
  계기의 "step 0이 250·500보다 2.7 dB 깨끗" 관측은 청취로 이어지지 않았다(계기 개선 ≠ 청취 개선). 화자 불일치인지 일반화
  한계인지는 정하지 않는다.
- 0029에서 계승 재구성(3·2)이 fresh(1·2)를 잡음 밖에서 앞선다 — **계승이 "효과 전무"가 아니라는 두 번째 관측**.
...
| 2026-09-07 | **말소리 성립 시점 = JSUT step 2,000 또는 그 이전 · 적응이 있던 능력을 지웠다.** 새 학습 없음. 페이지 A: 문장별 지속 성립 N=2,000(2k recon은 3/3), 품질은 2→5로 계속 상승(성립 ≠ 완성), 해제 전 이미 말소리(디코더 인과 미확정), **같은 2k 예산의 KO 런은 말소리 없음** (데이터·음역·시작점 동시 상이라 축 미확정). 페이지 B: **동결 JSUT 사전은 Arona 텍스트를 자연성 3~4로 읽고 2k 적응 뒤 같은 문장이 1이 된다(2/3)** → 적응이 있던 능력을 지움; 계승 posterior는 **step 0부터** 복원 못 함(warm-up 탓 아님, 계기 −2.7 dB는 청취로 이어지지 않음); 0029에서 계승 > fresh(잡음 밖). catch 잡음 바닥 A 0단계·B 1단계. 원본@2000이 말소리로 확인돼 replay@2000 비교가 다음 라운드에 유효. 청취 부담 축소 방침 등재. |
