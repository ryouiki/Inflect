...
세 문장 전부 같은 구조다: matched@500 추론은 결함 0의 말소리(0004에서는 **품질 3, "매우 어색한
한국어 발음을 하는 남성"** — 알아들을 수 있는 한국어), matched@1500 두 경로는 말소리 아님(품질 1,
`most_blurred` 2/2), control@10k 재구성은 콤이 심한 말소리(품질 2, 한국어 예).

후속 설계에 쓸 수 있는 결론(3/3 문장): (1) 콤의 크기와 말소리 성립은 별개다. (2) **릴리스 사전은
한국어 음소에서 이미 알아들을 수 있는 말을 만들고, linguistic adaptation의 첫 1,000 step이 그것을
없앤다.** (3) 이 디코더는 이 목소리의 posterior 잠재를 말로 렌더한다.

다음은 기존 체크포인트(control·K-A·K-B·E2, 1,000 step 간격 ~10k)에서 두 경로의 f0 잠김·콤 궤적을
재는 것이다 — "500과 10,000 사이에서 무엇이 성립하는가"를 새 학습 전에 기존 데이터로 묻는다.
K-A는 adversarial 항을 3,000까지 차단했으므로 그 step 1000이 "adversarial이 말소리 경로를
망가뜨리는가"의 무료 시험이다. **새 장기 학습은 그 뒤 별도로 승인받는다.**
