...
### 3.8 모듈 복원 — 어느 추론 모듈의 변화가 출력 저하에 기여했는가 (2026-09-07~08) — **원본 텍스트에서도 저하 확인, 모듈 귀속 미결**
...
**페이지 2(Arona 화자, 목표 텍스트, 11클립) 판정 대기** — `listening/restore-arona-20260907/`. 같은 라운드 안에 적응 전·후·복원 둘을 넣어
라운드 밖 참조를 쓰지 않는다. 규칙은 페이지 1과 동일하고, **언어 인식 저하는 실제 언어 응답 변화가 있을 때만** 덧붙이며(페이지 1에서
함께 나빠졌다고 묶지 않는다) **두 페이지 이득을 합산하지 않는다**. 이 뒤: 큰 회복 확인 → 그룹 갱신 제한 학습(저장소 코드 필요, 별도 계획);
미확인 → 모듈 귀속 미결로 남기고 **JSUT 동일 데이터 재시작 대조**(대상 데이터를 바꾸지 않아도 같은 재시작 조건에서 나빠지는가) 검토.
