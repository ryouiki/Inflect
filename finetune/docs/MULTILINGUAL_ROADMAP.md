...
### 3.2 외부 감사와 확정 결함 5건 (2026-09-05) — **수정 완료, 대조 실험 진행**
...
| 관측치 p50 | 실물 | legacy | matched | 차이 |
|---|---:|---:|---:|---:|
| 콤, 신호 대비 dB | −52.3 | −35.9 | **−39.0** | **−3.16** |
| 광대역 바닥, 신호 대비 dB | −52.0 | −42.2 | **−46.7** | **−4.64** |
| 레벨 dBFS | −20.0 | −36.4 | −32.5 | +3.97 |
| on/off 비율(스크린) | −0.1 | 6.1 | 7.7 | +1.63 |

문장별로 콤이 더 조용한 것 **37/40**, 바닥이 더 조용한 것 **40/40**, 스크린이 더 나쁜 것 **37/40**.

**수정은 두 물리량 모두를 개선했고 스크린이 반대로 보고했다.** 유효한 관측치로 4단 규칙을
적용하면 **부분 개선**(−3.16 dB, `partial` −1.5와 `large` −4.0 사이). 바닥 4.6 dB 개선은
"조용한 셀을 채우라는 기울기"가 실제로 광대역 바닥을 만들고 있었다는 예측의 직접 확인이다.
...
그리고 그 판정("악화")뿐이다. 계기 검사와 `grid_tone_level_db`·`off_grid_level_db`는 실험 후에
추가한 사후 탐색이며 사전 판정을 덮지 않는다. 문장별 원시값 120행(3체크포인트 × 40문장,
...
- **미시행 셀이 디코더를 사실상 면제한다.** 진단의 원래 주장은 한 방향으로는 확립할 수 없었고,
  완전 격자가 그것을 지지한다. §3.1의 유보("스왑 테스트가 디코더를 배제하지 못한다")는
  이제 시행 결과로 해소됐다.
- **디코더 학습은 상황을 악화시켰다**: 적응 잠재에서 D-KB > D-control > D-release.
  콤 레벨 지표에서는 D-control이 깨끗한 잠재에도 D-release보다 4.2 dB 나쁘다.
...
계기 수정은 C21로 등재했다. **콤 발생 시점도 좁혔다**: 사이드카가 step 500 −0.24 → step 550 4.76,
즉 linguistic 단계 시작 후 50 step 안이다.

한계: 단일 seed · 1,500 step · 폴리시 미진입 · 청취 없음.
