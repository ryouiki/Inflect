...
### 3.6 JSUT 기준점과 posterior 계승 (2026-09-06) — **측정 완료, 청취 대기**
...
**청취 대기**: `listening/jsut-baseline-20260906/`(JSUT 화자, 8트랙 × 3문장)와
`listening/s2-inherit-20260906/`(JA Arona 화자, 8트랙 × 3문장). **화자를 섞지 않는다.** 판정은 말소리 성립
(자연성 ≥ 2)과 **품질 유지**(자연성 원점수·결함·링잉 기술)를 **따로** 기록하고, B는 네 결과(이득/악화/둘 다
성립/막지 못함)를 모두 명시한다. C의 문구는 "이 JSUT 초반 조건에서 말소리·품질이 유지됐는지 관측했다"까지이며
**코드 원인의 배제도 확정도 아니다.**
