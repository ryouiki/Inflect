...
늦추지만 막지 못하며 붕괴의 필요조건이 아니다. **(2) 회복은 "학습 가능한 디코더 × adversarial ON"이
함께 있을 때만 일어난다.** 3000 이후 네 런이 2×2다: control·K-A(디코더 학습 + adv ON)는 4000에서
즉시 해제, K-B(디코더 학습 + adv OFF)는 6000까지 40/40 잠김이고 7,000 step 학습 뒤에도 infer 32/40,
E2(디코더 동결 + adv ON)는 끝까지 잠김. **(3) 콤 초과는 내내 +6~+12로 남는다.** 회복 뒤에도 +8이다.
...
표의 "해제"를 "말소리 회복"으로 읽지 않는다.** JA 런 E1(디코더 500 해제, adv ON)이 "동결 디코더 아래의
linguistic 단계가 붕괴를 만든다"의 무료 시험이라 같은 궤적을 재는 중이다(다른 화자, 측정만).
