...
### 3.9 동일 데이터 재시작 대조 — 데이터를 바꾸지 않아도 저하되는가 (2026-09-08) — **청취 대기**
...
청취 페이지(봉인, 판정 대기): `listening/restart-jsut-20260908/` — JSUT 화자·JSUT 텍스트, **처음 쓰는 문장 0012·0031**,
실물 / 적응 전 / Arona 데이터 적응 후 / JSUT 데이터 적응 후 + **주질문 트랙(`jsutdata`) 복제 catch** = 9클립. 사전 선언 규칙 3행:
① 적응 전 − JSUT 데이터 ≥ 2(두 문장) → 동일 데이터 재시작 후 저하, ② 두 조합의 비교는 **관측만** 적고 데이터 변경에 귀속하지 않음
(`--dataset`을 바꾸면 계승 posterior의 데이터 정합까지 함께 바뀐다), ③ 적응 전 − Arona 데이터 ≥ 2 → 페이지 1의 손상 확인을 새 문장에서
재확인. 언어 축은 **`예` → `아니오`만 악화**로 읽고 `예` → `판단 어려움`은 미결. catch 교체로 분류가 바뀌면 ①②는 미결, ③은 별도 판정.
