...
않는다.** 이 대조가 분해하지 못하는 것 하나 더: **R1이 도는 1,170행은 전부 base가 이미 학습한 행이므로**(base 런의 `train_jsonl_sha256`가 현재
파일과 동일) R1은 기학습 자료의 반복이고 S2-inherit은 미학습 화자 자료의 반복이다 — 저하가 없어도 **"적은 새 자료를 반복해도 무해"로 넓히지 않는다.**
...
- **여전히 열려 있는 것**: 이 대조는 자료량·반복 횟수·부분집합 구성·**기학습 여부**를 한 묶음으로 움직였다. **R1이 도는 행은 전부 base가 이미 학습한
  행**이므로 이 결과를 **"적은 새 자료를 반복해도 무해"로 넓히지 않는다.** 기전 확정 금지. 부분집합 추출 1회, 청취자 1인, 문장 2개.
